...
impl WorkspaceService {
    ...
    pub fn scan(&self) -> Result<Vec<DocumentEntry>> {
            ...
            if kind.is_dir() {
                if entry.depth() > 0 {
                    let relative = path
                        ...
                        .map_err(|_| StorageError::OutsideWorkspace)?;
        ...
        entries.sort_by(|left, right| {
            parent_slash(&left.path)
                .to_ascii_lowercase()
                .cmp(&parent_slash(&right.path).to_ascii_lowercase())
    ...
    pub fn write(&self, relative_path: &str, content: &str) -> Result<LoadedDocument> {
        ...
        if content.len() as u64 > MAX_DOCUMENT_BYTES {
            return Err(StorageError::TooLarge(content.len() as u64));
        }
    ...
    pub fn create(&self, relative_path: &str, kind: EntryKind) -> Result<()> {
        ...
        let target = self.secure_target(&relative)?;
        if target.exists() {
            return Err(StorageError::Conflict("同名文件或文件夹已存在".to_owned()));
        }
        match kind {
    ...
    pub fn rename(&self, source: &str, target: &str) -> Result<()> {
        ...
        if target_path.exists() {
            return Err(StorageError::Conflict("目标名称已存在".to_owned()));
        }
    ...
    pub fn import_file(&self, source: impl AsRef<Path>, target_directory: &str) -> Result<String> {
        ...
        let name = source
            .file_name()
            .ok_or_else(|| StorageError::InvalidPath(source.to_string_lossy().into_owned()))?;
    ...
    pub fn import_directory(
        ...
        if target_parent.starts_with(&source) {
            return Err(StorageError::InvalidPath(
                "不能把文件夹导入到自身内部".to_owned(),
            ));
        }
        let source_name = source
            .file_name()
            .unwrap_or_else(|| std::ffi::OsStr::new("导入文档"));
...
fn validate_relative(value: &str, allow_empty: bool) -> Result<PathBuf> {
    ...
    if source.is_absolute() {
        return Err(StorageError::InvalidPath(value.to_owned()));
    }
    ...
    if result.as_os_str().is_empty() && !allow_empty {
        return Err(StorageError::InvalidPath(value.to_owned()));
    }
    Ok(result)
}
...
mod tests {
    ...
    fn scans_empty_directories_and_reads_utf16() {
        ...
        assert!(entries
            .iter()
            .any(|entry| entry.path == "空文件夹/子目录" && entry.kind == EntryKind::Directory));
    ...
    fn rejects_symlink_escape() {
        ...
        let workspace = WorkspaceService::open(&root).unwrap();
        assert_eq!(
            workspace.write("escape/pwn.md", "no"),
            Err(StorageError::OutsideWorkspace)
        );
        fs::remove_dir_all(root).unwrap();
