    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::StaleRevision { expected, received } => {
                write!(f, "编辑事务版本过期：当前 {expected}，收到 {received}")
            }
            Self::OutOfBounds { range, chars } => write!(
                f,
                "编辑范围 {}..{} 超出文档字符数 {chars}",
                range.start, range.end
            ),
            Self::OverlappingOperations => f.write_str("同一事务包含重叠编辑范围"),
            Self::InvalidSelection { selection, chars } => write!(
                f,
                "选区 {}..{} 超出文档字符数 {chars}",
...
impl EditorDocument {
    ...
    pub fn next_undo_semantic(&self) -> Option<EditSemantic> {
        self.undo.back().map(|entry| entry.semantic)
    }
    ...
    pub fn char_to_byte(&self, character: usize) -> Result<usize, EditError> {
        if character > self.len_chars() {
            return Err(EditError::OutOfBounds {
                range: TextRange::caret(character),
    ...
    pub fn apply(&mut self, transaction: EditTransaction) -> Result<EditResult, EditError> {
        ...
        let mut indexed = transaction
            .operations
            .iter()
            .enumerate()
            .map(|(index, operation)| (index, operation.range()))
            .collect::<Vec<_>>();
    ...
    pub fn replace_selection(
        ...
        self.apply(EditTransaction::replace(
            self.revision,
    ...
    pub fn mark_saved(&mut self) -> EditResult {
...
mod tests {
    ...
    fn edits_unicode_by_character_offset() {
        ...
        document
            .replace_selection("原生", EditSemantic::Typing)
            .unwrap();
    ...
    fn rejects_stale_and_overlapping_transactions() {
        let mut document = EditorDocument::new("abcdef");
        document
            .replace_selection("x", EditSemantic::Typing)
            .unwrap();
        assert!(matches!(
            document.apply(EditTransaction::replace(
                0,
                TextRange::caret(0),
                "y",
                EditSemantic::Typing
        ...
        assert_eq!(
            ...
            Err(EditError::OverlappingOperations)
        );
    }
    ...
    fn undo_redo_and_saved_state_are_content_based() {
        ...
        document
            .replace_selection(" two", EditSemantic::Typing)
            .unwrap();
    ...
    fn composition_preedit_does_not_mutate_source_until_commit() {
        ...
        document
            .update_composition("zhong", Some(TextRange::new(0, 5)))
            .unwrap();
