...
pub struct ToolExecutionResult {
    ...
    pub output: String,
    #[serde(default)]
    pub text_sink_label: Option<String>,
}
...
pub enum AgentCoreError {
    ...
    ToolResultMismatch,
    MixedExclusiveToolCalls,
    InvalidTextSink,
    ToolCallDuringTextSink,
    MaximumRounds(usize),
    InvalidSseUtf8,
}
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidState(expected) => {
                write!(formatter, "Agent 状态错误，当前操作要求：{expected}")
            }
            Self::ToolResultMismatch => formatter.write_str("工具结果与待执行调用不匹配"),
            Self::MixedExclusiveToolCalls => {
                formatter.write_str("文档流式写入工具必须单独调用")
            }
            Self::InvalidTextSink => formatter.write_str(
                "文本写入通道只能由唯一的 exclusive 工具打开",
            ),
            Self::ToolCallDuringTextSink => {
                formatter.write_str("文档流式输出阶段不能继续调用工具")
            }
            Self::MaximumRounds(rounds) => {
                write!(formatter, "Agent 连续执行 {rounds} 轮工具仍未结束")
            }
            Self::InvalidSseUtf8 => formatter.write_str("SSE 事件不是有效 UTF-8"),
        }
    }
}
...
pub struct AgentTurnMachine {
    ...
    pending_sink: Option<String>,
}

impl AgentTurnMachine {
    pub fn new(
        system_prompt: impl Into<String>,
        history: impl IntoIterator<Item = RuntimeMessage>,
        max_rounds: usize,
    ) -> Self {
        let mut messages = vec![RuntimeMessage::text(
            RuntimeRole::System,
            system_prompt,
        )];
        ...
        Self {
            ...
            max_rounds: max_rounds.clamp(1, 16),
            rounds: 0,
    ...
    pub fn start(&mut self) -> Result<AgentAction> {
        if self.state != MachineState::Ready {
            return Err(AgentCoreError::InvalidState("尚未开始"));
        }
        self.request_completion(true, None)
    }

    pub fn accept_model(&mut self, response: ModelResponse) -> Result<AgentAction> {
        if self.state != MachineState::WaitingModel {
            return Err(AgentCoreError::InvalidState("等待模型响应"));
        }
        self.rounds += 1;

        if let Some(label) = self.pending_sink.take() {
            if !response.tool_calls.is_empty() {
                return Err(AgentCoreError::ToolCallDuringTextSink);
            }
            self.state = MachineState::WaitingSinkCommit;
            return Ok(AgentAction::CommitExclusiveText {
                label,
        ...
        let exclusive_count = response
            .tool_calls
            .iter()
            .filter(|call| call.exclusive_text_sink)
            .count();
        if exclusive_count > 0
            && (exclusive_count != 1 || response.tool_calls.len() != 1)
        {
            return Err(AgentCoreError::MixedExclusiveToolCalls);
        }

        self.final_content.push_str(&response.content);
        self.messages.push(RuntimeMessage {
        ...
        if response.tool_calls.is_empty() {
            ...
            return Ok(AgentAction::Completed {
    ...
    pub fn accept_tools(
        ...
        if self.state != MachineState::WaitingTools {
            return Err(AgentCoreError::InvalidState("等待工具结果"));
        }
        validate_tool_results(&self.pending_calls, &results)?;

        let sink_results = results
            .iter()
            .filter(|result| result.text_sink_label.is_some())
            .collect::<Vec<_>>();
        if !sink_results.is_empty()
            && (sink_results.len() != 1
                || self.pending_calls.len() != 1
                || !self.pending_calls[0].exclusive_text_sink)
        {
        ...
        if let Some(label) = sink_results
            .first()
            .and_then(|result| result.text_sink_label.clone())
        {
    ...
    pub fn accept_sink_commit(
        &mut self,
        summary: impl Into<String>,
    ) -> Result<AgentAction> {
        if self.state != MachineState::WaitingSinkCommit {
            return Err(AgentCoreError::InvalidState("等待文本写入提交"));
        }
        self.state = MachineState::Finished;
        Ok(AgentAction::Completed {
    ...
    pub fn cancel(&mut self, reason: impl Into<String>) -> AgentAction {
        self.state = MachineState::Finished;
        AgentAction::Failed {
            message: format!("Agent 已取消：{}", reason.into()),
            rounds: self.rounds,
        }
    }
    ...
    fn request_completion(
        ...
        Ok(AgentAction::RequestCompletion {
...
fn parse_sse_event(event: &[u8]) -> Result<Option<String>> {
    let value = std::str::from_utf8(event)
        .map_err(|_| AgentCoreError::InvalidSseUtf8)?;
    let data = value
        .lines()
...
mod tests {
    ...
    fn completes_direct_model_response() {
        ...
        assert!(matches!(
            ...
            AgentAction::RequestCompletion { round: 1, .. }
        ));
        assert_eq!(
            machine.accept_model(response("完成", Vec::new())).unwrap(),
    ...
    #[test]
    fn executes_tools_and_requests_the_next_round() {
        let mut machine = AgentTurnMachine::new("system", Vec::new(), 4);
        machine.start().unwrap();
        assert!(matches!(
            machine
                .accept_model(response("", vec![call("c1", false)]))
                .unwrap(),
            AgentAction::ExecuteTools { .. }
        ));
        assert!(matches!(
            machine
                .accept_tools(vec![ToolExecutionResult {
                    call_id: "c1".to_owned(),
                    output: "文档内容".to_owned(),
                    text_sink_label: None,
                }])
                .unwrap(),
        ...
        assert_eq!(machine.messages().last().unwrap().role, RuntimeRole::Tool);
    }

    #[test]
    fn exclusive_sink_gets_one_extra_tools_disabled_response() {
        let mut machine = AgentTurnMachine::new("system", Vec::new(), 1);
        machine.start().unwrap();
        machine
            .accept_model(response("", vec![call("writer", true)]))
            .unwrap();
        assert!(matches!(
            machine
                .accept_tools(vec![ToolExecutionResult {
                    call_id: "writer".to_owned(),
                    output: "准备写入".to_owned(),
                    text_sink_label: Some("note.md".to_owned()),
                }])
                .unwrap(),
            AgentAction::RequestCompletion {
                tools_enabled: false,
        ...
        assert_eq!(
            machine
                .accept_model(response("# 新文档", Vec::new()))
                .unwrap(),
            AgentAction::CommitExclusiveText {
                label: "note.md".to_owned(),
                content: "# 新文档".to_owned(),
            }
        );
        assert!(matches!(
            machine.accept_sink_commit("已写入").unwrap(),
            AgentAction::Completed { rounds: 2, .. }
        ));
    }

    #[test]
    fn rejects_mixed_exclusive_calls_and_fake_text_sinks() {
        let mut machine = AgentTurnMachine::new("system", Vec::new(), 4);
        machine.start().unwrap();
        assert_eq!(
            machine.accept_model(response(
                "",
                vec![call("writer", true), call("read", false)]
            )),
            Err(AgentCoreError::MixedExclusiveToolCalls)
        );

        let mut machine = AgentTurnMachine::new("system", Vec::new(), 4);
        machine.start().unwrap();
        machine
            .accept_model(response("", vec![call("read", false)]))
            .unwrap();
        assert_eq!(
            machine.accept_tools(vec![ToolExecutionResult {
                call_id: "read".to_owned(),
                output: "fake".to_owned(),
                text_sink_label: Some("note.md".to_owned()),
            }]),
            Err(AgentCoreError::InvalidTextSink)
        );
    }

    #[test]
    fn rejects_mismatched_results_and_enforces_max_rounds() {
        ...
        mismatch
            ...
            .unwrap();
        assert_eq!(
            mismatch.accept_tools(vec![ToolExecutionResult {
                call_id: "wrong".to_owned(),
                output: String::new(),
                text_sink_label: None,
            }]),
            Err(AgentCoreError::ToolResultMismatch)
        );
        ...
        limited
            ...
            .unwrap();
        assert_eq!(
            limited.accept_tools(vec![ToolExecutionResult {
                call_id: "read".to_owned(),
                output: "result".to_owned(),
                text_sink_label: None,
            }]),
            Err(AgentCoreError::MaximumRounds(1))
        );
    }
