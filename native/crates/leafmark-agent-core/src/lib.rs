    ...
    pub fn accept_tools(&mut self, results: Vec<ToolExecutionResult>) -> Result<AgentAction> {
        ...
        for result in &results {
            if let Some(label) = &result.text_sink_label {
                ...
                if sink_label.is_none() {
                    sink_label = Some(label.clone());
                }
            }
        }
        if sink_count != 0
...
mod tests {
    ...
    fn rejects_mixed_exclusive_calls_and_fake_sinks() {
        ...
        fake.start().unwrap();
        fake.accept_model(response("", vec![call("read", false)]))
            .unwrap();
        assert_eq!(
            fake.accept_tools(vec![ToolExecutionResult {
