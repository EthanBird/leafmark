...
impl DocumentArchive {
    pub fn load(root: impl AsRef<Path>) -> Result<Self> {
        ...
        let index = match fs::read(&index_path) {
            Ok(bytes) => match serde_json::from_slice::<ArchiveIndex>(&bytes) {
                ...
                Ok(index) => {
                    ...
                    ArchiveIndex { version: INDEX_VERSION, documents: index.documents }
                }
                Err(error) => {
                    backup_corrupt_index(&root, &index_path)?;
                    let _ = error;
                    ArchiveIndex { version: INDEX_VERSION, documents: Vec::new() }
                }
            },
        ...
        let archive = Self { root, documents_dir, index_path, index };
        archive.persist()?;
        Ok(archive)
    }
    ...
    pub fn entries(&mut self) -> Result<Vec<ArchiveEntry>> {
        ...
        entries.sort_by(|left, right| {
            right.last_opened_ms.cmp(&left.last_opened_ms)
    ...
    pub fn record(
        ...
        let canonical = source.canonicalize().map_err(|_| ArchiveError::InvalidSource)?;
        if !canonical.is_file() { return Err(ArchiveError::InvalidSource); }
        let source_path = canonical.to_string_lossy().into_owned();
        let key = path_key(&canonical);
        let position = if let Some(position) = self.index.documents.iter().position(|entry| path_key(Path::new(&entry.source_path)) == key) {
            position
        } else {
            let id = self.unique_id(&key);
            self.index.documents.push(ArchiveEntry {
                id,
                name: canonical.file_name().unwrap_or_default().to_string_lossy().into_owned(),
        ...
        if touch { entry.last_opened_ms = now_ms(); }
    ...
    pub fn open_source(&mut self, source: &Path) -> Result<ArchivedContent> {
        ...
        if !canonical.is_file() { return Err(ArchiveError::InvalidSource); }
        let key = path_key(&canonical);
        if let Some(id) = self.index.documents.iter()
        ...
        let entry = self.record(&canonical, &content, metadata.len(), metadata_modified_ms(&metadata), true)?;
        Ok(ArchivedContent { entry, content })
    }

    pub fn open(&mut self, id: &str) -> Result<ArchivedContent> {
        let position = self.index.documents.iter().position(|entry| entry.id == id).ok_or(ArchiveError::MissingEntry)?;
        ...
        let result = entry.clone();
        self.persist()?;
        Ok(ArchivedContent { entry: result, content })
    }
    ...
    pub fn vcs_snapshot(&self, id: &str) -> Result<(String, Vec<u8>)> {
        let entry = self.index.documents.iter().find(|entry| entry.id == id).ok_or(ArchiveError::MissingEntry)?;
        let bytes = fs::read(self.snapshot_path(id)).map_err(|_| ArchiveError::MissingSnapshot)?;
    ...
    pub fn set_favorite(&mut self, id: &str, favorite: bool) -> Result<Vec<ArchiveEntry>> {
        let entry = self.index.documents.iter_mut().find(|entry| entry.id == id).ok_or(ArchiveError::MissingEntry)?;
    ...
    pub fn remove(&mut self, id: &str) -> Result<Vec<ArchiveEntry>> {
        let position = self.index.documents.iter().position(|entry| entry.id == id).ok_or(ArchiveError::MissingEntry)?;
        if self.index.documents[position].favorite { return Err(ArchiveError::FavoriteEntry); }
    ...
    pub fn clear_history(&mut self) -> Result<Vec<ArchiveEntry>> {
        let removed = self.index.documents.iter().filter(|entry| !entry.favorite).map(|entry| entry.id.clone()).collect::<Vec<_>>();
        self.index.documents.retain(|entry| entry.favorite);
        for id in removed { remove_if_exists(&self.snapshot_path(&id))?; }
        self.persist()?;
        self.entries()
    }
    ...
    pub fn reveal_path(&self, id: &str) -> Result<PathBuf> {
        ...
        if source.is_file() { return Ok(source); }
        let snapshot = self.snapshot_path(id);
        if snapshot.is_file() { return Ok(snapshot); }
    ...
    pub fn rename_sources(&mut self, previous_root: &Path, next_root: &Path) -> Result<()> {
        ...
        for entry in &mut self.index.documents {
            ...
            let Ok(suffix) = source.strip_prefix(previous_root) else { continue };
            if !safe_relative_suffix(suffix) { continue; }
            let next = if suffix.as_os_str().is_empty() { next_root.to_path_buf() } else { next_root.join(suffix) };
            entry.source_path = next.to_string_lossy().into_owned();
            entry.name = next.file_name().unwrap_or_default().to_string_lossy().into_owned();
    ...
    pub fn snapshot_path(&self, id: &str) -> PathBuf { self.documents_dir.join(format!("{id}.md")) }

    fn unique_id(&self, key: &str) -> String {
        let base = format!("{:016x}", fnv1a64(key.as_bytes()));
        if !self.index.documents.iter().any(|entry| entry.id == base) { return base; }
        for index in 2..10_000 {
            let candidate = format!("{base}-{index}");
            if !self.index.documents.iter().any(|entry| entry.id == candidate) { return candidate; }
        }
        format!("{base}-{}", now_ms())
    }

    fn persist(&self) -> Result<()> {
        let bytes = serde_json::to_vec_pretty(&self.index).map_err(|error| ArchiveError::InvalidIndex(error.to_string()))?;
...
fn safe_relative_suffix(path: &Path) -> bool { path.components().all(|component| matches!(component, Component::Normal(_))) }
...
fn atomic_write(path: &Path, bytes: &[u8]) -> Result<()> {
    AtomicFile::new(path, AllowOverwrite).write(|file| file.write_all(bytes)).map_err(|error| ArchiveError::Io(error.to_string()))
}
...
fn backup_corrupt_index(root: &Path, index_path: &Path) -> Result<()> {
    if index_path.exists() { fs::copy(index_path, root.join(format!("index.corrupt-{}.json", now_ms())))?; }
    Ok(())
}
...
mod tests {
    ...
    fn temp_root(label: &str) -> PathBuf {
        let root = std::env::temp_dir().join(format!("leafmark-archive-{label}-{}-{}", std::process::id(), now_ms()));
    ...
    fn writes_exact_v1_index_and_reopens_existing_snapshot() {
        ...
        let entry = archive.record(&source, "# 保留副本", 14, 123, true).unwrap();
        let value: serde_json::Value = serde_json::from_slice(&fs::read(archive_root.join("index.json")).unwrap()).unwrap();
    ...
    fn corrupt_index_is_backed_up_and_recovered() {
        ...
        assert!(fs::read_dir(&archive_root).unwrap().flatten().any(|entry| entry.file_name().to_string_lossy().starts_with("index.corrupt-")));
        fs::remove_dir_all(root).unwrap();
    }
}
