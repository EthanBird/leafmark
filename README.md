...
LeafMark，中文名“一叶”，是从 DRPA 知识文档体验中独立出来的本地文档应用，支持 Windows、Linux 与 Android。Markdown、Word、Excel 与 PowerPoint 均可在本机编辑；PDF 使用本地只读查看。文档不会上传到云端。

## 已实现

- 本地目录直接作为文档库，不使用数据库或专有格式
- 本地打开并编辑 `.docx` / `.rtf`、`.xlsx` / `.xls` / `.xlsb` / `.ods` / `.csv`、`.pptx` / `.odp`；Office 模型在独立 Worker 中运行，首屏只取当前视口。旧版 `.doc` / `.ppt` 可从保留副本交给系统应用
- Office 以 OOXML 为原生格式写回：未修改部件保持原样，Microsoft Word / Excel / PowerPoint 可继续打开。外部来源只写 LeafMark 私有副本；文档库文件保存时同步更新工作区
- Excel 使用稀疏单元格、虚拟滚动与内置公式引擎；Word 支持段落/Run 级格式；PowerPoint 可编辑文本框并新增幻灯片
...
- Office 打开后先立即建立标签，再由专用 Worker 解析；Word 分段、表格按视口、演示按页传回。Excel 使用稀疏存储与虚拟网格，公式在 Worker 中重算
...
Office/PDF 的打开流水线见 [0.7.7 文档引擎](docs/DOCUMENT_ENGINE_0.7.7.md)；0.8 起 Word / Excel / PowerPoint 的编辑器方案见 [Office 编辑器](docs/OFFICE_EDITOR.md)。
