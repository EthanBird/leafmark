...
- Windows 资源管理器右键“打开方式”与 `.md` / `.markdown` 文件关联
- 单实例响应外部 Markdown：应用已运行时，右键打开会聚焦现有窗口并载入文档
- 最近打开与收藏；每次打开、保存都会同步保留独立 Markdown 副本
- 源文档被移动或删除后，历史与收藏仍可读取、导出保留副本
...
安装 Windows NSIS 版本后，LeafMark 会注册为 `.md` 和 `.markdown` 的可用编辑器。进入“设置 → 文档库 → 默认打开 Markdown”，点击“设置为默认”，然后在 Windows 默认应用页面中为 `.md` 选择 LeafMark。Windows 不允许应用静默替换用户的默认文件关联，因此最终选择由系统设置完成。

历史与收藏正文保存在 LeafMark 应用配置目录的 `archive` 文件夹，索引保存在 `history.json`。清空历史只删除未收藏的保留副本。
